function read_PISCOdata_ChannelIslands(meta_savename)
...
if ~exist(Meta.PISCO_filename,'file') % if the file doesn't exist
    Dir = 'data/';
    File = 'UCSB_to2017_noHeaders.csv'; % this version lacks headers
    
    fid = fopen(strcat([Dir,File]));
    D = textscan(fid, strcat(['%s %s ',...% campus  method
                          '%f %f %f ',... % year month day
                           '%s %s %s %s ',... % site side zone level
                           '%f ',... % transect
                           '%s ',... % classcode
                           '%f %f %f %f ',... % count, fish_tl, min_tl, max_tl
                           '%s ',... % observer
                           '%f %f %f ',... % depth vis temp
                           '%s %s ',... % surge windwave
                           '%f %f %f %f %f ',... % pctcnpy, macpyr1_4, macpyr5_15stipe, macpyrgt15stipe, nerluestipes
                           '%s ']),... % notes
    ...
    ColNames = {'campus','method','year','month','day','site','side','zone','level','transect',...
            ...
            'windwave','pctcnpy','macpyr1_4stipe','macpyr5_15stipe','macpyrgt15stipe','nerluestipes','notes'};
