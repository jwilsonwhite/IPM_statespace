...
F1 = cand_vec(1); % F in pre-MPA period
F2 = cand_vec(2); % F in post-MPA period
%F = repmat(F2,length(T),1); % expand into vector
F = [repmat(F1,length(Tpre),1); repmat(F1,length(Meta.Tpre2),1); repmat(F2,length(Meta.Tpost),1)];
...
% This is necessary if we are starting in pre-data years (pre2003 runs).
