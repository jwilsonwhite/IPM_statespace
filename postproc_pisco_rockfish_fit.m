...
% 'Chains' indicates the number of chains that were run, if MCMC chains
% were run manually in parallel. If unset, default to the number of chains
...
if exist('Chains','var') && ~isnan(Chains)
   ...
   for c = 1:length(Chains)
       load(strcat(Dir,fit_savename_tmp,'_',num2str(c),'.mat'))
       if c == 1 
       mc_str_tmp = mc_str;
       else
       mc_str_tmp(c) = mc_str;
